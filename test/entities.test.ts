...
// @Entity wraps the class and injects static query methods at runtime;
...
describe('Entities (Active Record — decorator style)', () => {
  ...
  describe('Decorator metadata', () => {
    ...
    it('@Column({ autoGenerated: true }) registra el campo como auto-generado', () => {
      expect(UserModel.autoGeneratedFields).toContain('id')
    })
  ...
  describe('saveEntity() — INSERT', () => {
    ...
    it('PKs de inserts consecutivos son distintos e incrementales', async () => {
      ...
      u2.age = 21
      u2.active = 1
      await Entities.save(u1)
      await Entities.save(u2)
  ...
  describe('deleteEntity()', () => {
    ...
    it('lanza error si no tiene PK', async () => {
      ...
      await expect(Entities.delete(user)).rejects.toThrow()
    })
  })
  ...
  describe('columnName — renombrado de columnas', () => {
    ...
    it('registra el columnsMap (propiedad → columna DB)', () => {
      expect(EmployeeModel.columnsMap).toEqual({
        id: 'employee_id',
    ...
    it('el primaryKey usa el columnName de la PK', () => {
      expect(EmployeeModel.primaryKey).toBe('employee_id')
    })

    it('fields conserva los nombres de propiedad', () => {
      expect(EmployeeModel.fields).toEqual(['id', 'firstName', 'isActive', 'email'])
    })
    ...
    it('DELETE elimina por la columna renombrada de la PK', async () => {
      ...
      await Entities.delete(employee)
