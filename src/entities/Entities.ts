...
export class Entities {
  ...
  static async save(entity: object): Promise<void> {
    ...
    const columnsMap: Record<string, string> = cls.columnsMap ?? {}
    ...
    if ((entity as any)[pkProp] != null) {
      ...
      try {
        ...
        if (autoGeneratedFields.includes(pkProp)) {
          ;(entity as any)[pkProp] = await conn.lastInsertId()
        }
      } finally {
        await conn.close()
      }
  ...
  static async delete(entity: object): Promise<void> {
    ...
    const columnsMap: Record<string, string> = cls.columnsMap ?? {}
    const pkProp = Object.keys(columnsMap).find((p) => columnsMap[p] === pk) ?? pk
    const pkValue = (entity as any)[pkProp]
